    ...
    >
      ...
      <div className="flex-grow overflow-auto p-4">
        {messages.map((message, index) => (
          <div key={index} className={`flex ${message.sender === 'AI' ? 'justify-start' : 'justify-end'} my-2`}>
            <div className={`max-w-[70%] rounded-lg px-4 py-2 ${message.sender === 'AI' ? 'bg-gray-200' : 'bg-blue-100'}`}>
              {message.content}
            </div>
          </div>
        ))}
